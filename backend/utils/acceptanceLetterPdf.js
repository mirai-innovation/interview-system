...
export function streamAcceptanceLetterPdf(res, user, application, programType = 'MIRI') {
  ...
  if (programType === 'EMFUTECH') {
    return streamEMFUTECHAcceptanceLetterPdf(res, user, application);
  }
...
/**
 * Streams the EMFUTECH acceptance letter PDF to res.
 * Same layout/format as the MIRI letter, but program/subject/registration link
 * are replaced for EMFUTECH.
 * @param {object} res - Express response
 * @param {object} user - User document (plain or mongoose)
 * @param {object} application - Application document (plain or mongoose)
 */
function streamEMFUTECHAcceptanceLetterPdf(res, user, application) {
  const fullName =
    application?.firstName && application?.lastName
      ? `${application.firstName} ${application.lastName}`
      : user.name;
  ...
  const regCode =
    user.digitalId ||
    `EMFUTECH-2026-01-${String(user._id).slice(-3).padStart(3, "0")}`;
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="Acceptance_Letter_EMFUTECH_${fullName.replace(/\s+/g, "_")}.pdf"`
  );
  doc.pipe(res);
  ...
  doc
    ...
    .fontSize(11)
    .text(
      "Subject: Official Final Decision for EMFUTECH Program 2026",
      { align: "right", width: textWidth }
    );
  doc.moveDown(1.5);
  ...
  doc
    .fillColor("blue")
    .text("https://www.mirai-innovation-lab.com/emfutech-registration/", {
      link: "https://www.mirai-innovation-lab.com/emfutech-registration/",
      continued: false,
    })
    .fillColor("black");
